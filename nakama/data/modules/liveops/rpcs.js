var RPCSystem = {
    rpcAdminStartSeason: function(ctx, logger, nk, payload) {
        ...
        const season = SeasonsSystem.startSeason(nk, input.id, input.name, input.duration);
    ...
    rpcEquipCosmetic: function(ctx, logger, nk, payload) {
        ...
        const success = CosmeticsSystem.equipCosmetic(nk, ctx.userId, input.slot, input.cosmeticId);
        if (!success) {
