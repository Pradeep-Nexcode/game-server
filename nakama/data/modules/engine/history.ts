...
export interface PlayerHistoryState {
    ...
    rotation: Vector3;
    hp: number;
    // We can add hitbox radius here if it varies, but usually it's static config
}

export interface HistorySnapshot {
    tick: number;
    players: { [id: string]: PlayerHistoryState };
}
...
export function getHistoryRange(state: GameState, startTick: number, endTick: number): HistorySnapshot[] {
    return state.history.filter(frame => frame.tick >= startTick && frame.tick <= endTick);
}

export function getRewoundPlayers(state: GameState, targetTick: number): { [id: string]: PlayerHistoryState } | null {
...
function interpolatePlayers(
    prev: { [id: string]: PlayerHistoryState },
    next: { [id: string]: PlayerHistoryState },
    t: number
): { [id: string]: PlayerHistoryState } {
    const result: { [id: string]: PlayerHistoryState } = {};
